...
struct FerryView: View {
    ...
    private var bundle: Bundle {
        guard let code = languageCode, let path = Bundle.main.path(forResource: code, ofType: "lproj") else {
            return Bundle.main
        }
    ...
    var body: some View {
        let binding = Binding<FerryPier>(
            get: { self.viewModel.state?.from ?? .Tsawwassen },
        ...
        return ZStack {
            VStack(spacing: 0) {
                VStack(alignment: .leading) {
                    HStack {
                        ...
                        Spacer()
                        Text(localizedDay)
                        Button(self.languageCode != "fr" ? "FR" : "EN") {
                            let lang = (self.languageCode != "fr" ? "fr" : "en")
            ...
            if showPicker {
                Picker("DEPART FROM", selection: binding) {
